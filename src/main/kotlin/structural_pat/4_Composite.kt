 ...
 * ChatGPT 说：
 *
 * 组合模式（Composite Pattern）是一种结构型设计模式，核心目标是“将对象组合成树形结构以表示‘部分-整体’层次结构”。
 * 使得客户端对单个对象和组合对象的使用具有一致性.
 *
 * 关键在于组合的构造函数参数s和所实现的接口是相同的,这里可以体现kotlin的支持
 * 而桥接则是将所实现的接口拆分出小的接口由构造函数参数实现
 *
 * 组合的整体与部分可以认为相同,一滴牛奶与一杯牛奶
 * 桥接则是有机结构,一辆汽车内部的离合器,刹车,轮胎和这辆车
 */
fun main() {
